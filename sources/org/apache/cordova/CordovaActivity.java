...
public class CordovaActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        LOG.m15i(TAG, "Apache Cordova native platform version 6.4.0 is starting");
        LOG.m9d(TAG, "CordovaActivity.onCreate()");
        if (!this.preferences.getBoolean("ShowTitle", false)) {
            getWindow().requestFeature(1);
        }
        if (this.preferences.getBoolean("SetFullscreen", false)) {
            LOG.m9d(TAG, "The SetFullscreen configuration is deprecated in favor of Fullscreen, and will be removed in a future version.");
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    dlg.setPositiveButton(str3, new C01531());
                    dlg.create();
                    dlg.show();
                } catch (Exception e) {
                    CordovaActivity.this.finish();
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        try {
            ...
            LOG.m9d(TAG, "JSONException: Parameters fed into the method are not valid");
            e.printStackTrace();
        }
    }
}
