...
public class PluginManager {
    ...
    public void exec(String service, String action, String callbackId, String rawArgs) {
        ...
        if (plugin == null) {
            LOG.m9d(TAG, "exec() call to unknown plugin: " + service);
        ...
        try {
            ...
            if (duration > ((long) SLOW_EXEC_WARNING_THRESHOLD)) {
                LOG.m21w(TAG, "THREAD WARNING: exec() call to " + service + "." + action + " blocked the main thread for " + duration + "ms. Plugin should use CordovaInterface.getThreadPool().");
            }
            if (!wasValidAction) {
            ...
            LOG.m13e(TAG, "Uncaught exception from plugin", e2);
